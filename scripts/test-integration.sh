...
[[ $(docker inspect --format '{{range .Config.Env}}{{println .}}{{end}}' tobari-opa | grep -Fxc 'GOMEMLIMIT=256MiB') == 1 ]] || fail "OPA does not constrain its Go heap with headroom below the container memory ceiling"
