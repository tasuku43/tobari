...
14. **Standard authentication is native, Workspace-owned, and post-policy.** The release surface has no provider binding, projection, handle, vault, root key, companion, or Auth Broker activation path. Agent CLIs create and persist their own credentials only inside one Workspace home; host credentials are never inherited. Gateway redacts client authentication and cookies before OPA/audit and forwards them unchanged only after allow. The research surface may compile the closed Broker research boundary: a Workspace then receives only a versioned project-bound opaque handle, never the brokered primary secret, renewable session, root key, or temporary role credential. Runtime input cannot widen either compile-time surface. Root keys, vaults, provider acquisition, companion sessions, handle issuance/resolution, and credential-bearing types remain infrastructure-owned and research-only.
