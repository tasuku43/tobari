...
14. **Standard authentication is native, Workspace-owned, and post-policy.** The standard profile has no provider binding, projection, handle, vault, root key, companion, or Auth Broker activation path. Agent CLIs create and persist their own credentials only inside one Workspace home; host credentials are never inherited. Gateway redacts client authentication and cookies before OPA/audit and forwards them unchanged only after allow. The experimental development profile may compile the closed Broker research boundary: a Workspace then receives only a versioned project-bound opaque handle, never the brokered primary secret, renewable session, root key, or temporary role credential. Runtime input cannot widen either compile-time profile. Root keys, vaults, provider acquisition, companion sessions, handle issuance/resolution, and credential-bearing types remain infrastructure-owned and experimental.
...
9. Provider-specific policy operations, arbitrary executable adapters, PAT
   discovery, and multiple-account selection remain excluded from standard.
   Standard exposes no provider extension boundary or helper-backed auth; users
   run the provider CLI's native login inside the Workspace. The experimental
   profile retains the strict schema-v1 static provider manifest, protected
