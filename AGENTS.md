...
2. **Discovery and action stay separate.** A `RoleDiscover` command may return candidates and opaque references. A `RoleAct` uses exactly one target-binding mode: it requires a unique opaque reference and accepts it unchanged, declares one complete command-bound `tool_local` fixed target when the command path alone identifies a CLI-owned singleton, or declares one optional Context reference whose omission resolves only from the installation-owned current Context selector. A current-Context fallback never consults CWD, Workspace, name, or labels, and an explicit override remains invocation-local. Fixed-target reads and writes produce and consume no references. A fixed-target create consumes none and may produce only confirmed child-resource references whose kinds differ from the fixed creation-scope kind. An action does not rediscover, normalize, decode, or reconstruct an identifier. Required-reference chains must lead back to an invocable producer rather than a closed cycle.
...
14. **Standard authentication is native, Context-Home-owned, and post-policy.** The release surface has no provider binding, projection, handle, vault, root key, companion, or Auth Broker activation path. Agent CLIs create and persist their own credentials only inside one Context Home, shared by every Workspace owned by that Context; host credentials are never inherited. Gateway redacts client authentication and cookies before OPA/audit and forwards them unchanged only after allow. The research surface may compile the closed Broker research boundary: a Workspace then receives only a versioned project-bound opaque handle, never the brokered primary secret, renewable session, root key, or temporary role credential. Runtime input cannot widen either compile-time surface. Root keys, vaults, provider acquisition, companion sessions, handle issuance/resolution, and credential-bearing types remain infrastructure-owned and research-only.
...
The Context-authentication slice follows it through `internal/app/authcmd`,
...
2. Classify the command with `RoleUtility`, `RoleDiscover`, or `RoleAct`; for an act, choose exactly one reference-bound, command-bound `tool_local` fixed, or typed current-Context-fallback target, and declare any reference kinds on structured inputs and output fields so produced/consumed edges are derived.
...
8. Declare `Effect`, `Intent`, `TargetRef`, and `Impact`. Bind create scope through `MutationContract.parent_input`; bind a write's existing target through `target_id_input` and any distinct scope through optional `parent_input`. A Context-aware command may instead make that exact typed input optional and declare `current_context_fallback`; resolve omission only through the persisted selector and keep an explicit override local to the invocation. Make missing, unbound, mismatched, or inconsistent values fail before the side effect.
9. Provider-specific policy operations, arbitrary executable adapters, PAT
   discovery, and multiple-account selection remain excluded from the release surface.
   The release surface exposes no provider extension boundary or helper-backed auth;
   standard authentication uses the provider CLI's native login inside the Workspace.
   The research surface retains the strict schema-v1 static provider manifest, protected
   non-terminal stdin import, and the closed GitHub, Datadog, OpenAI,
   Anthropic, Chatwork, and AWS plans. Owner manifests cannot select helpers.
   Extend the
...
10. Add unit, contract, opaque-reference round-trip, negative-path,
    ...
    Authentication changes additionally prove stdin-only secret ingress,
    terminal-input refusal before reading,
