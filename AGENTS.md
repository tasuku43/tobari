...
14. **Brokered credentials remain closed, post-policy, and project-bound.** A Workspace may receive only a versioned opaque handle bound to its stable Context, project, provider, credential revision, and exact HTTPS/header or signing binding; it never receives the brokered primary secret, renewable session, root key, or temporary role credential. Gateway removes a recognized handle, performs non-secret introspection, asks OPA about the ordinary HTTP effect, applies exactly one same-revision static resolution, fixed Datadog/OpenAI refresh result, or bounded AWS SigV4 result only after allow, and never forwards or falls back with an invalid handle. Owner-authored provider manifests remain strict non-secret schema-v1 stdin-import data, not executable adapters or policy. The standard reviewed built-ins are the compiled GitHub, Datadog, OpenAI/Codex, Anthropic/Claude, and Chatwork plans; the experimental repository profile additionally activates the reviewed AWS plan, and runtime input cannot widen either profile. Managed profiles remain unsupported; owner manifests cannot select helpers, dynamic records, refresh, signing, companions, or executables. Root keys, vaults, provider acquisition, companion sessions, handle issuance/resolution, and credential-bearing types remain infrastructure-owned.
...
9. Provider-specific policy operations, arbitrary executable adapters, PAT
   ...
   limited to the active closed reviewed built-in profile: standard GitHub,
   Datadog, OpenAI, and Anthropic, plus AWS only in the experimental repository
   profile; Chatwork remains static/import-only. Owner manifests cannot
   select helpers. Extend the
