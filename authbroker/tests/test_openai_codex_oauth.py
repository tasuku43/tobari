...
class OpenAICodexOAuthStateTests(unittest.TestCase):
    ...
    def test_rejects_noncanonical_state_and_wrong_executable_binding(self) -> None:
        ...
        invalid = {
            ...
            "invalid observed version": state_bytes(version="latest"),
            "prerelease observed version": state_bytes(version="0.147.0-beta.1"),
            "leading-zero observed version": state_bytes(version="00.147.0"),
    ...
    def test_accepts_stable_observed_versions_under_one_driver_contract(self) -> None:
        for version in ("0.145.0", "0.146.0", "0.147.0", "1.0.0", "12.345.6789"):
            with self.subTest(version=version):
                state = CodexOAuthState.parse(
                    state_bytes(version=version),
                    driver_revision=DRIVER_REVISION,
                )
                self.assertEqual(
                    state.document["codex_executable"]["version"], version
                )
