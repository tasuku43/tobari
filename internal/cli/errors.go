...
func (c *CLI) fail(ctx context.Context, err error) int {
	...
	if invocationErrorFormat(ctx) == errorFormatJSON {
		...
		output = renderTextErrorForProgram(payload, humanStyleAllowed(ctx, c, c.Err), c.catalog.programName())
	}
...
func renderTextErrorWithColor(payload errorPayload, color bool) []byte {
	return renderTextErrorForProgram(payload, color, ProgramName)
}
...
func recoveryCommand(command string) string {
	return recoveryCommandForProgram(ProgramName, command)
}

func recoveryCommandForProgram(program, command string) string {
	if program == "" {
		program = ProgramName
	}
	if command == program {
