...
func installationMigrationApplySpec() CommandSpec {
	...
	errors := mutationCommandErrors(installationmigrationcmd.TaskApply, installationmigrationcmd.TaskPlan,
		...
		declaredCommandError(fault.KindUnavailable, "installation_migration_failed", false, "doctor", "Inspect exact installation state."),
	)
...
func runInstallationMigrationPlan(ctx context.Context, c *CLI, command CommandSpec, _ operation.Intent, inputs ParsedInputs) int {
	...
	text := []byte(fmt.Sprintf("Installation migration plan %s\nSource generation %d\nTemplates %d\nContexts %d\nWorkspaces %d\n", plan.PlanRef, plan.SourceGeneration, plan.TemplateCount, plan.ContextCount, plan.WorkspaceCount))
	...
	output, err := finalAuthorityOutput(command.Path, "installation_migration_plan", projection, format, text)
	if err != nil {
...
func runInstallationMigrationApply(ctx context.Context, c *CLI, command CommandSpec, intent operation.Intent, inputs ParsedInputs) int {
	...
	text := []byte(fmt.Sprintf("Installation migration applied\nGeneration %d\nRevision %s\n", result.ActiveGeneration, result.ActiveRevision))
	...
	output, err := finalAuthorityOutput(command.Path, "installation_migration", projection, format, text)
	if err != nil {
