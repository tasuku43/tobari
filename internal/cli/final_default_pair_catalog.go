...
func finalDefaultPairEnterErrors() []CommandError {
	return append(finalNoOutputMutationErrors(WorkspaceEntryCommandPath, "status"),
		...
		classifiedCommandError(fault.KindRejected, "current_context_required", false, fault.PhasePrecondition, fault.ChangeNone, "context list", "Discover a Context reference, then select it with context use."),
		classifiedCommandError(fault.KindRejected, "context_in_use", false, fault.PhasePrecondition, fault.ChangeNone, "workspace list", "Use the Context's existing Workspace or select an unattached Context."),
		classifiedCommandError(fault.KindNotFound, "context_not_found", false, fault.PhaseObservation, fault.ChangeNotApplicable, "context list", "Discover current Context authority."),
		classifiedCommandError(fault.KindUnavailable, "context_read_failed", false, fault.PhaseObservation, fault.ChangeNotApplicable, "context list", "Read the current Context selection again."),
		classifiedCommandError(fault.KindContract, "invalid_context", false, fault.PhaseVerification, fault.ChangeUnknown, "context list", "Repair contradictory current Context authority."),
		...
		classifiedCommandError(fault.KindContract, "invalid_customized_template_body", false, fault.PhasePrecondition, fault.ChangeNone, "help tobari", "Inspect the supported first-use Template customization contract."),
		...
		classifiedCommandError(fault.KindUnavailable, "workspace_entry_repair_required", true, fault.PhasePrecondition, fault.ChangeNone, "tobari", "Repeat root entry so readiness and the staged recovery flow can reconcile the exact current Workspace."),
		...
		classifiedCommandError(fault.KindUnavailable, "workspace_entry_busy", true, fault.PhasePrecondition, fault.ChangeNone, "status", "Read current authority, then retry after the blocking Workspace session or exclusive Context Home operation finishes."),
...
func finalDefaultPairStatusSpec() CommandSpec {
	fields := []OutputField{
		...
		{Name: "template_state", Type: OutputFieldTypeString, Description: "Whether a Workspace-selected or prospective default Template is present.", Enum: []string{"absent", "selected"}},
		{Name: "template", Type: OutputFieldTypeObject, Description: "Selected Workspace Template, or the prospective default when no Workspace exists.", Nullable: true, Fields: statusHomeTemplateFields()},
		...
		{Name: "attention", Type: OutputFieldTypeArray, Description: "Separately ordered pending facts.", SemanticScope: "Every selected-scope permission, Service, or final-authority recovery fact in the snapshot.", Items: &OutputField{Type: OutputFieldTypeObject, Description: "One attention fact.", Fields: []OutputField{{Name: "kind", Type: OutputFieldTypeString, Description: "Attention kind.", Enum: []string{"permissions", "services", "mutation_recovery"}}, {Name: "count", Type: OutputFieldTypeInteger, Description: "Relevant pending count."}, {Name: "observation", Type: OutputFieldTypeString, Description: "Typed observation state."}, {Name: "path", Type: OutputFieldTypeString, Description: "Exact Catalog drill-down path."}, {Name: "inputs", Type: OutputFieldTypeArray, Description: "Typed drill-down inputs.", SemanticScope: "Every input required by the drill-down command.", Items: &OutputField{Type: OutputFieldTypeObject, Description: "One exact input.", Fields: []OutputField{{Name: "name", Type: OutputFieldTypeString, Description: "Catalog input name."}, {Name: "value", Type: OutputFieldTypeString, Description: "Validated value."}}}}}}},
	}
	return CommandSpec{
		...
		Agent: AgentContract{
			CapabilityID: "workspace.authority", Outcome: "Return one CWD-first snapshot of desired, active, applied, observed, attention, and next-action facts for the selected Workspace and its bound Context",
