...
func authLoginSpec() CommandSpec {
	provider := CommandInput{
		...
		Description:   "Credential provider to authenticate. Omission opens an interactive provider selector. Each current reviewed built-in has one supported Workspace client tool, displayed and selected automatically: github uses GitHub CLI (gh); aws uses AWS CLI (aws); datadog uses pup; openai uses Codex 0.146.0; anthropic uses Claude Code 2.1.220. AWS --method selects acquisition inside the aws pairing, not another tool.",
	...
	return CommandSpec{
		...
		Agent: AgentContract{
			...
			Prerequisites: []string{
				...
				"The reviewed GitHub CLI, AWS CLI, pup, Codex 0.146.0, or Claude Code 2.1.220 executable is available through the trusted-host PATH from a conventional installation root (/bin, /usr/bin, /usr/local, /opt/homebrew, /opt/local, /nix/store, or /snap); project, temporary, and home-local executable paths are rejected.",
				"The caller has interactive terminal streams on stdin and stderr and can complete the selected github, aws, datadog, openai, or anthropic provider flow on the trusted host.",
			...
			Errors: authMutationErrors("auth login",
				...
				declaredCommandError(fault.KindUnavailable, "openai_cli_unavailable", false, "auth login", "Install the reviewed Codex 0.146.0 CLI on the trusted host and retry."),
				...
				declaredCommandError(fault.KindUnavailable, "anthropic_cli_unavailable", false, "auth login", "Install the reviewed Claude Code 2.1.220 CLI on the trusted host and retry."),
				declaredCommandError(fault.KindRejected, "anthropic_login_cancelled", false, "auth login", "Retry the trusted-host Anthropic login when ready."),
				declaredCommandError(fault.KindRejected, "anthropic_login_timeout", false, "auth login", "Start a new Claude setup-token login and complete it within the bounded window."),
				declaredCommandError(fault.KindUnavailable, "anthropic_login_failed", false, "auth login", "Retry the isolated trusted-host Claude login after inspecting the failure."),
