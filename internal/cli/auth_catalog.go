...
func authLoginSpec() CommandSpec {
	return CommandSpec{
		Path: "auth login", Summary: "Authenticate one Context with a built-in provider helper",
		...
		Agent: AgentContract{
			...
			Inputs:       []CommandInput{authProviderInput("Built-in provider helper to use; this slice supports github."), executionContextInput(), formatInput()},
			Output:       authResultOutput(),
			Prerequisites: []string{
				"The selected Context exists.",
				"The shared Auth Broker is already running, ready, and unlocked.",
				"The caller has interactive terminal streams on stdin and stderr and can complete the provider helper's trusted-host interaction.",
			},
			...
			Errors: authMutationErrors("auth login",
				...
				declaredCommandError(fault.KindInvalidInput, "auth_login_tty_required", false, "help auth login", "Run trusted-host GitHub login from an interactive terminal."),
