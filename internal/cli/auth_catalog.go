...
func authLoginSpec() CommandSpec {
	return CommandSpec{
		...
		Agent: AgentContract{
			...
			Inputs: []CommandInput{
				authProviderInput("Built-in provider helper to use: github runs the reviewed gh device flow; aws runs the reviewed AWS IAM Identity Center device flow."),
			...
			Prerequisites: []string{
				...
				"The caller has interactive terminal streams on stdin and stderr and can complete either the github device flow or the aws IAM Identity Center device flow on the trusted host.",
				"For aws, the caller knows the access-portal start URL, SSO region, 12-digit account ID, and role name; request region remains ordinary Context or command configuration.",
			},
