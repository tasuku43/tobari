...
// attachment-local transport without exposing it through this port.
...
func (s *Service) Wait(ctx context.Context, id string) (tobari.PermissionWaitResult, error) {
	...
	if err := tobari.ValidatePermissionWaitID(id); err != nil {
		return "", fault.Wrap(fault.KindNotFound, "invalid_permission_wait", "permission wait is unavailable", false, err)
	}
