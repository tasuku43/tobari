...
func recoveryFor(id doctor.CheckID, status doctor.CheckStatus) *doctor.Recovery {
	...
	action := map[doctor.CheckID]string{
		...
		doctor.CheckIDCredentials:           "Repair unsafe managed-credential file ownership or modes.",
		doctor.CheckIDCredentialConfig:      "Repair the managed-credential configuration.",
