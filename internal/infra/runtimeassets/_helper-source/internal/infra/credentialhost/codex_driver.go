...
var codexSemanticVersionPattern = regexp.MustCompile(`^(0|[1-9][0-9]{0,5})\.(0|[1-9][0-9]{0,5})\.(0|[1-9][0-9]{0,5})$`)

var (
	ErrCodexExecutable     = errors.New("Codex CLI executable is invalid")
	ErrCodexVersion        = errors.New("Codex CLI identity output is invalid")
...
// CodexDriver owns one reviewed trusted-host Codex ChatGPT OAuth acquisition
// contract. Product version is observed for audit; fixed execution and strict
// state postconditions decide compatibility.
...
func (d *CodexDriver) Login(
	...
	observedVersion, err := d.observeCodexVersion(ctx, canonical, digest, home, environment)
	if err != nil {
...
func (d *CodexDriver) observeCodexVersion(
	ctx context.Context,
	executable string,
	digest string,
	home string,
	...
	version, ok := parseCodexVersion(stdout.bytes())
	if !ok {
		return "", ErrCodexVersion
	}
...
func (d *CodexDriver) acquireCodexCredential(
	...
	credential, err := newCodexCredential(executable, digest, observedVersion, auth, accountLabel)
	if err != nil {
		return CodexCredential{}, ErrCodexAuthCapture
	}
	return credential, nil
...
func parseCodexVersion(output []byte) (string, bool) {
	framed := output
	...
	if !bytes.HasPrefix(framed, []byte(codexVersionPrefix)) {
		return "", false
	}
	version := string(framed[len(codexVersionPrefix):])
	if !codexSemanticVersionPattern.MatchString(version) {
		return "", false
	}
	return version, true
}
