...
func contextMutationFault(err error) error {
	...
	case errors.Is(err, tobari.ErrWorkspaceEntryReconciliationConfirmed):
		return fault.WithClassification(fault.Wrap(fault.KindUnavailable, "workspace_entry_attachment_unavailable", "Workspace entry reconciliation is confirmed, but the interactive attachment did not start", false, err, fault.NextAction{Command: "context show", Reason: "Read the confirmed desired, applied, and active authority before another explicit entry."}), fault.PhaseAttachment, fault.ChangeConfirmed)
	case errors.Is(err, tobari.ErrWorkspaceEntryInterrupted):
		return fault.WithClassification(fault.Wrap(fault.KindUnavailable, "workspace_entry_interrupted", "Workspace entry reconciliation requires exact same-Context recovery", false, err, fault.NextAction{Command: "context show", Reason: "Read the preserved last-successful entry and active recovery authority."}), fault.PhaseMutation, fault.ChangePartial)
	...
	case errors.Is(err, tobari.ErrWorkspaceEntryPolicyMemoryInactive):
		return fault.WithClassification(fault.Wrap(fault.KindRejected, "workspace_entry_policy_memory_inactive", "Workspace entry requires the current Policy Memory activation", false, err, fault.NextAction{Command: "context show", Reason: "Read current and active Policy Memory authority before explicit policy reconciliation."}), fault.PhasePrecondition, fault.ChangeNone)
	case errors.Is(err, tobari.ErrWorkspaceEntryObservationUnavailable):
		return fault.WithClassification(fault.Wrap(fault.KindUnavailable, "workspace_entry_observation_unavailable", "Workspace entry prerequisites could not be observed exactly", false, err, fault.NextAction{Command: "context show", Reason: "Read desired, applied, and active authority without reconciling it."}), fault.PhaseObservation, fault.ChangeNotApplicable)
	case errors.Is(err, tobari.ErrWorkspaceEntryCanceledBeforeDecision):
		return fault.WithClassification(fault.Wrap(fault.KindCanceled, "workspace_entry_canceled", "Workspace entry was canceled before a durable reconciliation decision", false, err, fault.NextAction{Command: "context show", Reason: "Read current authority before deciding whether to enter again."}), fault.PhasePrecondition, fault.ChangeNone)
	default:
		return err
	}
}
