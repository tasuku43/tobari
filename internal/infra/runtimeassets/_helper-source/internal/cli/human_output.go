...
func (o *humanOutput) next(command, reason string) {
	o.nextForProgram(ProgramName, command, reason)
}

func (o *humanOutput) nextForProgram(program, command, reason string) {
	padded := fmt.Sprintf("%-*s", humanOutputLabelWidth, "Next")
	fmt.Fprintf(
		&o.Buffer, "  %s %s %s\n",
		applyStyleToken(o.color, styleMuted, padded),
		applyStyleToken(o.color, styleAccent, recoveryCommandForProgram(program, command)),
