...
func TestCodexLoginObservesVersionAndUsesNativeBrowserFlowEnvironmentAndCanonicalState(t *testing.T) {
	...
	wantEncoded := fmt.Sprintf(
		`{"schema_version":1,"codex_executable":{"path":%q,"sha256":%q,"version":"0.147.0"},"auth":%s}`,
...
func TestCodexLoginAcceptsStableObservedVersionsWhenPostconditionsSucceed(t *testing.T) {
	for _, version := range []string{"0.145.0", "0.146.0", "0.147.0", "1.0.0", "12.345.6789"} {
		t.Run(version, func(t *testing.T) {
			runner := &fakeCodexRunner{run: func(call int, _ context.Context, command Command) error {
				switch call {
				case 0:
					_, err := command.Stdout.Write([]byte(codexVersionPrefix + version + "\n"))
					return err
				case 1:
					return os.WriteFile(filepath.Join(environmentValue(t, command.Env, "CODEX_HOME"), codexAuthFileName), codexAuthFixture(t, codexAccountCanary, false), 0o600)
				default:
					t.Fatalf("unexpected command call %d", call)
					return nil
				}
			...
			if err != nil || credential.AccountLabel() != codexAccountCanary || runner.callCount() != 2 {
				t.Fatalf("credential=%v error=%v calls=%d", credential, err, runner.callCount())
			}
			encoded, err := credential.Encode()
			if err != nil || !bytes.Contains(encoded, []byte(`"version":"`+version+`"`)) {
...
func TestCodexLoginRejectsMalformedOrAmbiguousVersionBeforeLogin(t *testing.T) {
	tests := map[string]struct {
		stdout string
		stderr string
	}{
		"leading text":    {stdout: "version codex-cli 0.147.0\n"},
		"two lines":       {stdout: "codex-cli 0.147.0\nextra\n"},
		"stderr":          {stdout: "codex-cli 0.147.0\n", stderr: "warning\n"},
		"missing patch":   {stdout: "codex-cli 0.147\n"},
		"prerelease":      {stdout: "codex-cli 0.147.0-beta.1\n"},
		"leading zero":    {stdout: "codex-cli 00.147.0\n"},
		"too many digits": {stdout: "codex-cli 1234567.0.0\n"},
		"empty":           {stdout: ""},
	}
	...
	for name, mutateCall := range map[string]int{"after version": 0, "after login": 1} {
		t.Run(name, func(t *testing.T) {
			...
			runner := &fakeCodexRunner{run: func(call int, _ context.Context, command Command) error {
				switch call {
				case 0:
					_, _ = command.Stdout.Write([]byte(codexVersionPrefix + testCodexVersion + "\n"))
				case 1:
					codexHome := environmentValue(t, command.Env, "CODEX_HOME")
...
func TestDecodeCodexCredentialRejectsNonCanonicalOrChangedEnvelope(t *testing.T) {
	...
	tests := map[string][]byte{
		...
		"invalid observed version": []byte(strings.Replace(string(encoded), `"version":"0.147.0"`, `"version":"latest"`, 1)),
...
func testCodexExecutable(t *testing.T) string {
	...
	if err := os.WriteFile(path, []byte("synthetic Codex executable"), 0o700); err != nil {
		t.Fatal(err)
	}
	return path
}
