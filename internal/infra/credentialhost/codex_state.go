...
// AuthJSON returns the captured host-login auth.json, not the handle-only
// Workspace compatibility projection. Callers must treat it as secret.
...
func newCodexCredential(
	...
	credential := CodexCredential{payload: codexCredentialPayload{
		...
		Executable: codexExecutable{
			Path: executablePath, SHA256: executableDigest, Version: observedVersion,
		},
...
func validateCodexCredentialPayload(payload codexCredentialPayload) error {
	if payload.SchemaVersion != codexCredentialSchemaVersion ||
		!codexSemanticVersionPattern.MatchString(payload.Executable.Version) ||
