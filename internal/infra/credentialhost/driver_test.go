...
func TestLoginUsesExactCommandPrivateHomeAndPacksCache(t *testing.T) {
	...
	runner.run = func(_ context.Context, command Command) error {
		...
		assertPrivatePath(t, filepath.Join(home, ".aws"), 0o700)
		assertPrivatePath(t, filepath.Join(home, ".aws", "config"), 0o600)
		assertPrivatePath(t, filepath.Join(home, ".aws", "credentials"), 0o600)
		assertPrivatePath(t, cachePath(home), 0o700)
	...
	for _, operation := range []string{"login", "refresh"} {
		t.Run(operation, func(t *testing.T) {
			...
			loginRunner := &fakeRunner{run: func(_ context.Context, command Command) error {
				writeCacheFile(
					t, cachePath(environmentValue(t, command.Env, "HOME")),
...
func TestLoginAndRefreshFailClosedWhenPrivateHomeCleanupFails(t *testing.T) {
	...
	loginRunner := &fakeRunner{run: func(_ context.Context, command Command) error {
		writeCacheFile(t, cachePath(environmentValue(t, command.Env, "HOME")), testCacheName, testCacheContent("login-cache-secret"))
		return nil
	}}
	loginDriver := NewDriver(loginRunner)
	loginDriver.tempRoot = t.TempDir()
