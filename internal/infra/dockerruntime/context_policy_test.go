...
func TestContextPolicySnapshotUsesContextOwnedLayout(t *testing.T) {
	...
	if _, err := runtime.CreateContext(context.Background(), "policy-check", tobari.BuiltinImageSelector, tobari.ManifestPolicyModeGuided, tobari.ManifestSourceAccessReadWrite); err != nil {
		t.Fatal(err)
	}
