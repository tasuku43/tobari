...
func TestDecodeBrokerControlResponseAcceptsOnlyOperationSpecificSuccessFrames(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "issue Anthropic handle", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","oauth_scopes":["future:capability","user:inference"]}`},
...
func TestDecodeBrokerControlResponseRejectsCrossOperationAndAmbiguousFrames(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Anthropic issue unsorted scopes", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","oauth_scopes":["user:inference","future:capability"]}`},
