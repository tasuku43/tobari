...
func TestDecodeBrokerControlResponseAcceptsOnlyOperationSpecificSuccessFrames(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "issue Anthropic handle", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","oauth_scopes":["future:capability","user:inference"],"claude_subscription_type":"max","claude_rate_limit_tier":"example_claude_tier"}`},
...
func TestDecodeBrokerControlResponseRejectsCrossOperationAndAmbiguousFrames(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Anthropic issue missing scopes", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}`},
		{name: "Anthropic issue unsorted scopes", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","oauth_scopes":["user:inference","future:capability"],"claude_subscription_type":"max","claude_rate_limit_tier":"example_claude_tier"}`},
		{name: "Anthropic issue missing entitlement", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","oauth_scopes":["future:capability","user:inference"],"claude_subscription_type":"max"}`},
		{name: "Anthropic issue unsafe entitlement", args: []string{"issue_handle", "--provider", "anthropic"}, response: `{"schema_version":1,"ok":true,"provider":"anthropic","revision":"revision_synthetic","handle":"tobari-h1_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","oauth_scopes":["future:capability","user:inference"],"claude_subscription_type":"max\nother","claude_rate_limit_tier":"example_claude_tier"}`},
...
func TestBrokerControlLoginExpectationsRequireExactProviderShape(t *testing.T) {
	...
	for _, arguments := range [][]string{
		...
		// Workspace Manifest is the public identity, but the closed predecessor
		// Broker wire keeps its versioned context-id field. Do not create an
		// undocumented wire alias during the public vocabulary cutover.
		{"login", "--manifest-id", testBrokerContextID, "--provider", "github", "--account-label", "octocat"},
