...
type finalGatewaySettlementRunner struct {
	candidate     finalGatewaySettlementCandidate
	workspace     *tobari.WorkspacePolicyPrincipalAuthority
	workspaceNet  string
	selected      bool
	composeCalls  int
	policyEffects int
...
func (r *finalGatewaySettlementRunner) Output(_ context.Context, args, _ []string) ([]byte, error) {
	...
	if r.workspace != nil && len(args) >= 4 && args[0] == "container" && args[1] == "inspect" {
		payload, _ := json.Marshal(finalWorkspaceContainerObservation{
			ID: strings.Repeat("7", 64), Owner: ownerValue, Component: "tobari", Workspace: string(r.workspace.WorkspaceID),
			Role: projectWorkRole, Spec: string(r.workspace.AppliedEntry.ResolvedSpec), Running: true, Health: "healthy",
		})
		return payload, nil
	}
	if r.workspace != nil && len(args) >= 4 && args[0] == "inspect" && args[len(args)-1] != gatewayContainer && args[len(args)-1] != opaContainer {
		payload, _ := json.Marshal(map[string]map[string]string{r.workspaceNet: {"IPAddress": "172.30.0.2"}})
		return payload, nil
	}
	if r.workspace != nil && len(args) >= 5 && args[0] == "network" && args[1] == "inspect" {
		format := args[3]
		switch {
		case strings.Contains(format, ownerLabel):
			return []byte(ownerValue + "\n"), nil
		case strings.Contains(format, tobariIDLabel):
			return []byte(string(r.workspace.WorkspaceID) + "\n"), nil
...
func finalGatewayCoordinatorFixture(t *testing.T, workspaceIDs ...tobari.WorkspaceID) (*Runtime, *finalGatewaySettlementRunner, finalGatewaySettlementJournal) {
	...
	t.Setenv("TOBARI_UPSTREAM_TIMEOUT_SECONDS", "")
	root := t.TempDir()
	runner := &finalGatewaySettlementRunner{}
	...
	if plan.Contexts[0].Principal != nil {
		...
		principals = append(principals, FinalWorkspacePrincipalRow{
			...
			Presentation: authority.Presentation, ProjectRoot: authority.ProjectRoot, ContainerID: strings.Repeat("7", 64),
			ResolvedSpec: authority.AppliedEntry.ResolvedSpec, WorkspaceIP: "172.30.0.2", GatewayIP: "172.30.0.1", Network: network,
		})
		gatewayNetworks = append(gatewayNetworks, FinalGatewayNetworkAddress{Name: network, Address: "172.30.0.1"})
