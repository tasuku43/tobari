...
func (r *Runtime) ObservePermissionDisposition(
	...
	expression := `[result | revision := http.send({"method":"get","url":"http://127.0.0.1:8181/v1/data/tobari/aggregate_revision"}); revision.status_code == 200; decision := http.send({"method":"post","url":"http://127.0.0.1:8181/v1/data/tobari/http/decision","headers":{"content-type":"application/json"},"body":{"input":` + string(encoded) + `}}); decision.status_code == 200; object.get(decision.body, "result", null) != null; result := {"revision":revision.body.result,"decision":decision.body.result}][0]`
