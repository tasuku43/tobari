...
func TestVerifyGatewayImageRejectsOldAPIBeforeEngineMutation(t *testing.T) {
	...
	runner.metadata = strings.Replace(runner.metadata, `"io.tobari.gateway-api":"3"`, `"io.tobari.gateway-api":"2"`, 1)
	runtime := &Runtime{runner: runner}
...
func gatewayMetadata(architecture, repoDigest string) string {
	...
	return `{"RepoDigests":` + repoDigests + `,"Architecture":"` + architecture + `","Os":"linux","Config":{"User":"1000:1000","Labels":{"io.tobari.gateway-api":"3","io.tobari.gateway-role":"enforcement"},"Entrypoint":["/opt/tobari/entrypoint.sh"]}}`
}
