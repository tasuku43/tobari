...
func TestSameCanonicalRootCanOwnIndependentTobariInDifferentContexts(t *testing.T) {
	...
	if _, err := runtime.CreateContext(context.Background(), "restricted", tobari.BuiltinImageSelector, tobari.ManifestSourceAccessReadWrite); err != nil {
		t.Fatal(err)
	}
...
func TestBoundContextManifestSelectsSameRootWorkspaceWithoutNameRediscovery(t *testing.T) {
	...
	if _, err := runtime.CreateContext(context.Background(), "toolbox", tobari.BuiltinImageSelector, tobari.ManifestSourceAccessReadOnly); err != nil {
		t.Fatal(err)
	}
