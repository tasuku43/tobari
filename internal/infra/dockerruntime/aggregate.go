...
func (r *Runtime) readAggregateContextsWithTransactions(
	...
	for _, summary := range list.Items {
		...
		contextData["guardrail"] = map[string]any{
			...
			"graphql_baseline_grants": preset.GraphQLBaselineGrants,
...
func aggregateRouter(items []aggregateContext) ([]byte, error) {
	...
	builder.WriteString("preset_graphql_root_granted(root_field) if { some rule in data.tobari_contexts[input.principal.context_id].guardrail.graphql_baseline_grants; rule.scheme == input.request.authority.scheme; rule.host == input.request.authority.host; rule.port == input.request.authority.port; rule.method == input.request.method; rule.path == input.request.path.raw; rule.graphql_operation_type == input.request.graphql.operation_type; rule.graphql_root_field == root_field }\n")
