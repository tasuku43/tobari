...
func NewPolicyCandidate(denial PolicyDenial) (PolicyCandidate, error) {
	...
	sum := sha256.Sum256([]byte(strings.Join(
		[]string{
			"tobari-policy-candidate-v1", denial.Host, denial.Method, denial.Path,
		},
		"\x00",
	)))
	return PolicyCandidate{
		ID: "pcy_" + hex.EncodeToString(sum[:16]), ObservedAt: denial.Timestamp,
		Host: denial.Host, Method: denial.Method, Path: denial.Path,
...
func learnedRuleID(
	match, host, method, path string, examples, sourceCandidates []string,
) string {
	material := strings.Join(
		[]string{
			"tobari-learned-rule-v1", match, host, method, path,
...
func NewExactLearnedPolicyRule(candidate PolicyCandidate) (LearnedPolicyRule, error) {
	...
	rule := LearnedPolicyRule{
		Match: PolicyMatchExact, Host: candidate.Host, Method: candidate.Method,
		Path: candidate.Path, Examples: []string{candidate.Path},
		SourceCandidates: []string{candidate.ID},
	}
	rule.ID = learnedRuleID(
		rule.Match, rule.Host, rule.Method, rule.Path, rule.Examples, rule.SourceCandidates,
	)
	return rule, nil
}
...
func (r LearnedPolicyRule) Validate() error {
	...
	if r.ID != learnedRuleID(
		r.Match, r.Host, r.Method, r.Path, r.Examples, r.SourceCandidates,
	) {
...
func (r LearnedPolicyRule) Matches(host, method, path string) bool {
	if r.Host != host || r.Method != method {
		return false
	}
	switch r.Match {
...
func compactionID(host, method, prefix string, sourceRuleIDs []string) string {
	material := strings.Join(
		[]string{
			"tobari-policy-compaction-v1", host, method, prefix,
...
func (c PolicyCompaction) Validate() error {
	...
	if c.ID != compactionID(c.Host, c.Method, c.PathPrefix, c.SourceRuleIDs) {
...
func PolicyCompactions(rules []LearnedPolicyRule) ([]PolicyCompaction, error) {
	...
	for _, rule := range rules {
		...
		key := rule.Host + "\x00" + rule.Method + "\x00" + prefix
	...
	for _, group := range groups {
		...
		item := PolicyCompaction{
			Host: group[0].Host, Method: group[0].Method, PathPrefix: prefix,
			SourceRuleIDs: sourceRuleIDs, Examples: examples,
			OutsideCanary: outsidePrefixCanary(prefix),
		}
		item.ID = compactionID(item.Host, item.Method, item.PathPrefix, item.SourceRuleIDs)
...
func CompactLearnedPolicyRules(
	...
	prefixRule := LearnedPolicyRule{
		Match: PolicyMatchPrefix, Host: selected.Host, Method: selected.Method,
		Path: selected.PathPrefix, Examples: append([]string{}, selected.Examples...),
		SourceCandidates: sourceCandidates,
	}
	prefixRule.ID = learnedRuleID(
		prefixRule.Match, prefixRule.Host, prefixRule.Method, prefixRule.Path,
