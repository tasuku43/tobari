...
historical binding never triggers reconstruction, pull, or silent rebinding of
its execution image. Separate preparation tests prove that already-verified
candidate helpers cause no helper-source build, while absent candidate helpers
are fully extracted and validated from the fixed current source-addressed image
before any durable Workspace decision.
...
- CLI tests fix routing, help, rendering, exit behavior, the catalog-owned typed
  ...
  same-root different-Context create reachability and Context-reference-visible
  disambiguation,
...
| One Workspace per `(ContextID, canonical ProjectRoot)` with arbitrary sibling Workspaces per Context | Domain duplicate-index validation, exact Context binding checks, repeated create convergence, same-root/different-Context cases, shared Context Home/defaults, and concurrent convergence |
