...
- Typed-policy projection tests prove a Workspace Template owns only the exact
  `policy/domains/<host>/{allow,deny}.json` tree, any stale executable source
  fails closed, the fixed evaluator makes first brokered use learnable until an
  exact rule is installed, and the complete internal bundle is layout-checked
  and schema-checked. Source transaction tests exercise
...
- The human permission path is exercised through `review permissions`; its TTY
  ...
  retained denial. Neither path exposes or requires hand-editing the evaluator.
  Redirected
