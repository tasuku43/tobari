...
- Auth Broker tests cover exact 64 KiB schema-1 framing, bounded credential
  ...
  and rejection of every unknown record/operation. Provider network tests use
...
| Closed broker plan boundary | Exact static and dynamic record schemas, immutable renewable-adapter, persisted-record-contract, and control-login-plan membership, adapter/plan incapability tests, record/Vault import compatibility, shared Broker-owned login commit and snapshot/single-flight/barrier/CAS conformance, Context/project/provider/revision/HTTPS-header or signing introspection, bounded AWS/Datadog/OpenAI behavior, rotation/revocation, durable barriers, and no invalid-handle fallback |
