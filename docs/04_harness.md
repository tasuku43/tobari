...
| Context composition and selection | Stable-ID manifest/domain tests, required immutable source-access, preset origin/revision, and explicit native-readiness selection, creation-only defaults, legacy preset-coupled compatibility without rewrite, snapshot digest binding, catalog effect/target contracts, owner-only atomic store tests, current-default-only selection, explicit synthetic/persisted observation state, first-use zero-write and concurrent/read-only XDG canaries, permanent Tobari binding, unsupported-version fixtures, and agent-readiness transcript |
...
| Context policy-preset guardrail | Normalized owner-only snapshot/digest tests, source-change immutability, single-catalog current/history readiness validation, orthogonal enabled/disabled binary-readiness replacement without snapshot rewrite, stale active-revision invalidation and entry recovery, offline/GET-only/exact-ceiling terminal canaries, system-evaluator precedence over baseline/learned/Advanced policy, scheme-aware exact learned identity, and terminal-denial zero candidate/DNS/Broker/upstream counters |
