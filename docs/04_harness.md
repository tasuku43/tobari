...
| Context composition and selection | Stable-ID manifest/domain tests, required immutable source-access, Context policy revision, and explicit native-readiness selection, creation-only defaults, argument-free terminal create wizard with valid name/source/complete method decisions and zero-mutation cancellation/non-TTY/JSON canaries, composed snapshot filtering/digest binding, direct-mode compatibility, vertical list-card presentation, one typed `context show` fixture with concise and detailed goldens, one typed `context create` fixture with legacy and shared-summary goldens, complete method-override, create-cluster-state, detailed-inspection, and Context-preserving-next-action answer keys, byte-identical JSON with or without `--details` and unchanged create JSON, catalog effect/input contracts, owner-only atomic store tests, current-default-only selection, explicit synthetic/persisted observation state, first-use zero-write and concurrent/read-only XDG canaries, permanent Tobari binding, unsupported-version fixtures, and agent-readiness transcript |
...
| Context creation is one reviewed interaction | Raw-mode entry/restore and alternate-screen entry/exit count tests, six-stage transcript coverage with always-present ready-Runtime and Workspace-bootstrap steps after Network, Runtime/Bootstrap Back navigation, zero host reads when Bootstrap continues unchanged, explicit read-only candidate selection with unavailable reasons, complete effective network/runtime/filesystem/bootstrap review in flow order, inherited/override/reset controls, scroll coverage, section-local editing, line-mode fallback, standalone Context-preserving root continuation, semantic-drift re-review, and cancellation-before-mutation canaries |
