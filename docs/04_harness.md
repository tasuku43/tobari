...
2.96.0 cases fragment its complete no-newline browser prompt and exact
non-interactive device line, and accept only the exact host-open-only device
target or the fixed GitHub.com client, standard
HTTPS-login scope ceiling, state shape, and `127.0.0.1/callback` redirect. The
device path proves one browser open, zero listener calls, no Enter, and no
Workspace browser execution. Runtime checks fix the exact-default-argv wrapper,
pinned real executable, fixed login/setup argv, and pass-through. The fallback and
