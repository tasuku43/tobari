...
| Context Workspace bootstrap boundary | Closed AWS IAM Identity Center profile/session schema, semantic digest/generation/diff tests, fixed `~/.aws/config` regular-file and size bounds, unknown/duplicate/helper/credential/cache/alternate-path/symlink rejection, secret-free reports, staged-preview source-drift rejection, atomic Context replacement, exact private `.aws/config` bytes, create-before-publication rollback, no credentials/cache projection, legacy manifest compatibility, existing-Workspace byte preservation, and `not_configured`/`not_applied`/`current`/`older` status coverage |
