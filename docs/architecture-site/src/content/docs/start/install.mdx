...
For this source snapshot, build `bin/tobari-dev`, inspect `version --format json`, and use the development resolver until compatible immutable Gateway API 4 and Auth Broker API 3 indexes are published. `doctor` then checks the host without creating or repairing product state. The provenance shown in the page footer identifies the exact product snapshot used by the generated contracts and fixed source links.
