...
このソース時点では `bin/tobari-dev` を作り、`version --format json` で互換性を確認します。Gateway API 4 と Auth Broker API 3 に対応する不変な公開成果物が揃うまでは、開発用リゾルバーを使います。`doctor` は製品状態を作成・修復せずにホストを確認します。ページ下部に表示する生成元情報から、生成された契約と固定リンクが参照する製品スナップショットを確認できます。
