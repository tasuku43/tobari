...
| HTTP permission workflow | `policy candidates`, `policy review`, `policy rules`, `policy allow`, `policy deny`, `policy reset`; `policy preset list`, `policy preset show`, `policy preset init`, `policy preset validate` | Exact learned rules and explicit human batch review remain below one immutable Context guardrail. The internal `policy apply-reviewed` completion stays catalog-owned but is not a public command. Built-in and strict local custom presets replace implicit initial policy. `policy compactions` and `policy compact` are removed. No automatic retry follows allow. |
