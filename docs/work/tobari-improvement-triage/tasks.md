...
- [x] Record the auth-broker deferral packet with a machine-checkable main/branch boundary E2E. Evidence: [auth-broker-deferral](../auth-broker-deferral/goal.md); it is parked and deliberately excluded from current-wave commits and integration.
...
- [x] Preserve all unrelated user changes and the detached auth-broker branch. Evidence: current `codex/first-wave` status contains only the uncommitted auth-broker and coordinator packets; `codex/auth-broker` is not merged.
...
- [x] Generated diff and repository status are understood without absorbing unrelated changes. Evidence: the first-wave branch contains only scoped child commits; current uncommitted paths are limited to `docs/work/auth-broker-deferral/` and this coordinator packet.
