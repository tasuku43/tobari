...
- [x] Record the auth-broker deferral as a coordinator register decision with a machine-checkable main/branch boundary E2E. Evidence: `context.md` records the deferred row, `codex/auth-broker` is not an ancestor of `main`, and only the docs-only governance packet is on the current line.
- [x] Create the README/architecture/publication packet after the policy and catalog inputs are stable; require clean-checkout docs and journey replay E2E. Evidence: `quickstart-runtime-docs` and `architecture-publication` record their bounded replays and scoped commits `d98e086` and `001c4a7`.
...
- [x] Define the packet-audit evidence table and the disposition of every non-empty active/historical packet; require packet lifecycle E2E before cleanup. Evidence: [work-packet-retirement](../work-packet-retirement/goal.md); initial scoped commit `92d742c` and current-main refresh `1cc400e`.
- [x] Review durable-decision impact before closing the completed documentation children. Evidence: both packets record that no numbered contract or ADR change was required; the remaining publication and runtime blockers stay in their evidence packets.
...
- [x] Every completed current-wave child reports an intentional scoped commit after rerunning its E2E and required gates. Evidence: `7d096bb`, `8e1adfa`, `912c602`, `a007059`, `92d742c`, `831c7e7`, `001c4a7`, and `d98e086`; unresolved environment blockers remain explicitly recorded in the affected child packets.
- [x] The parked auth-broker implementation remains detached, unmerged, and unscheduled until explicit maintainer resumption. Evidence: `codex/auth-broker` is not an ancestor of `main`; its implementation paths are absent from `main`, while the governance packet is intentionally committed docs-only.
...
- [x] Generated diff and repository status are understood without absorbing unrelated changes. Evidence: `main` is clean after the scoped first-wave and second-wave commits; the detached auth implementation remains outside the current line.
