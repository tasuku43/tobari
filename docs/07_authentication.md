...
trusted binary projects the bundles as ordinary exact rules for every Context
...
rewriting the Context snapshot. One dedicated compile-time
...
Every request outside the immutable Context policy snapshot plus the selected
binary readiness overlay remains denied or reviewable. The policy applies to
every process in the Context, never to an executable name.
