...
whose immutable origin is exactly `builtin/agent-ready`; bundle and executable
names never enter policy and custom presets cannot select them. New snapshots
omit native readiness, while projection removes all historical bundle rules
from legacy snapshots before adding the current set. Updating Tobari therefore
updates readiness without recreating the Context. One dedicated compile-time
...
Every request outside the immutable snapshot plus selected binary readiness
overlay remains denied or reviewable under
