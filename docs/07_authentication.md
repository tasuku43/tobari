...
home; predecessor identity never authorizes Broker or native state.
...
trusted binary projects the bundles as ordinary exact rules for every Workspace Template
...
rewriting the Workspace Template snapshot. One dedicated compile-time
...
Every request outside the immutable Workspace Template policy snapshot plus the selected
binary readiness overlay remains denied or reviewable. The policy applies to
every process in the Workspace Template, never to an executable name.
...
self-update, and unmatched effect continues through ordinary Workspace Template policy.
...
under ordinary Workspace Template policy.
...
permission: SSO OIDC, portal, and AWS API effects remain under ordinary Workspace Template
...
The final Template/Context authority store has no predecessor decoder or public
migration command. A fixed-path, presence-only guard rejects legacy research
Broker or Workspace-authority state before the first final publication. It
does not read, adopt, move, delete, or reinterpret predecessor bytes. The user
must reset or recreate the unpublished development installation out of band;
later research use requires an explicit fresh Context-bound login or import.
...
flag, Runtime source/revision, Workspace Template revision, Workspace state, or renamed
...
bin/tobari-research auth login --context CONTEXT_REF --provider aws --method identity-center
bin/tobari-research auth login --context CONTEXT_REF --provider aws --method console
```
...
in a fresh mount-free container from the selected Workspace Template image. Tobari binds
...
2.1.220 at `/usr/local/bin/claude` in the selected Workspace Template image. A fresh
...
Context, its bound Template, installed static provider, and ready Broker are validated before the
secret is sent.
...
provider, and Workspace activation state. `template show` reports the same
...
Managed Gateway profiles, runtime-selected helpers, arbitrary executable
...
The Workspace Template revision contains no vault path, root key, primary secret, or
handle. Stable ContextID selects the separate encrypted vault.

Vaults use AES-256-GCM with a random 12-byte nonce and associated data binding
schema plus stable ContextID. Root keys stay in host storage and broker
...
1. Derive stable Context authority from the kernel-observed source
...
4. Remove one recognized handle and ask Broker for non-secret introspection of
   the full Context/provider/revision/target/header binding.
