...
A typed Workspace Manifest bootstrap snapshot is not authentication state. The AWS
...
Domain Model V1 migration preserves every standard Workspace home and native
authentication byte without reading, moving, or transforming it. Preserving a
predecessor Context UUID as WorkspaceManifestID never authorizes Broker state
or imports it into standard authentication.

Login creates no policy rule. The fixed agent-ready compatibility baseline
grants only a finite exact set of client-bootstrap and core effects,
independently from account scope or credential state. It includes these observed
native-login effects for the
pinned clients:
...
| TWG CLI device auth | `POST` | `https://auth.atlassian.com/oauth/device/code` |
| TWG CLI device token | `POST` | `https://auth.atlassian.com/oauth/token` |
| TWG CLI token revoke | `POST` | `https://auth.atlassian.com/oauth/revoke` |
| TWG CLI accessible sites | `POST` | `https://api.atlassian.com/accessible-products` |
| TWG CLI current-user identity | GraphQL `query` / `me` | `https://api.atlassian.com/graphql` |
| TWG CLI stable manifest | `GET` | `https://teamwork-graph.atlassian.com/cli/manifest.json` |
| pup DCR registration | `POST` | `https://api.datadoghq.com/api/v2/oauth2/register` |
| pup token exchange/refresh | `POST` | `https://api.datadoghq.com/oauth2/v1/token` |

The compile-time review sources for these exact rules are named
`claude_ready`, `codex_ready`, `gh_ready`, `twg_ready`, and `pup_ready` and are
coupled to Claude Code 2.1.220, Codex 0.147.0, GitHub CLI 2.96.0, TWG CLI 1.2.5,
and pup 1.10.7 respectively. TWG and pup are not part of the canonical base;
their readiness contracts apply when the pinned client is supplied by a custom runtime. The installed
trusted binary projects the bundles as ordinary exact rules for every Workspace Manifest
whose native readiness is enabled; bundle and executable names never enter
policy and users cannot select them as a profile. New snapshots omit native
readiness rules, while projection removes all historical bundle rules before
adding the current set. Updating Tobari therefore updates readiness without
rewriting the Workspace Manifest snapshot. One dedicated compile-time
family catalog separates pinned client version from an independent positive
contract revision and retains append-only contract history. Candidate traffic
cannot extend it. A running
aggregate generated by an older catalog remains unchanged until explicit
`cluster up`; status marks it invalid and root entry refuses it with that exact
recovery command.

Browser navigation occurs on the host and is not a Workspace egress grant.
Every request outside the immutable Workspace Manifest policy snapshot plus the selected
binary readiness overlay remains denied or reviewable. The policy applies to
every process in the Workspace Manifest, never to an executable name.
...
For GitHub CLI 2.96.0, the attached host process accepts only a dedicated opener
request from the native GitHub.com login paths. Its strict callback-bearing
fallback invokes the opener immediately. The preferred
device flow accepts only the exact
`https://github.com/login/device` target, verifies the selected owned
Workspace, and opens it once on the host only after GitHub CLI invokes the
attachment-scoped `GH_BROWSER` after native Enter. It creates no callback listener. Tobari
does not read, retain, relay, validate, or render the one-time code. The opener
request supplies only bounded framing; strict URL validation supplies
authority.
...
GitHub.com HTTPS web/device login with GitHub CLI prompting and the
attachment-owned opener as its Workspace browser. GitHub CLI therefore displays its
one-time code and fixed device URL, waits for native Enter, invokes the opener,
persists its own credential, and then
...
self-update, and unmatched effect continues through ordinary Workspace Manifest policy.

For TWG CLI 1.2.5, the attached host process recognizes only a dedicated
schema-v1 opener request carrying an exact
`https://auth.atlassian.com/oauth/activate?user_code=<synthetic-shape>` target.
The URL must contain exactly one bounded unreserved `user_code` value and no
userinfo, explicit port, alternate path or host, encoding, extra query key, or
fragment. Tobari verifies the selected owned Workspace. It opens that target
once on the host after TWG's native browser confirmation and creates no callback
listener. Malformed, duplicated, oversized, or changed requests open nothing. TWG owns the code, polling,
exchange, refresh, credential persistence, and result presentation. Tobari
does not log or persist the code or URL. `twg_ready` contract revision 2 grants
only the exact device-code, token, revoke, accessible-products, stable-manifest,
and GraphQL `query` / `me` effects above. Its
declared endpoint accepts the client's absent `Content-Length` only under the
generic fixed transport and 1 MiB semantic caps; it does not grant ordinary
HTTP, mutation, sibling roots, or mixed-root documents. The stable manifest
grant includes no beta manifest, installer, checksum, artifact, download, or
update execution. Atlassian REST, telemetry, and unmatched effects remain under ordinary
...
only to the selected owned Workspace. Pup owns state and PKCE validation, DCR
interpretation, exchange, refresh, persistence, and presentation. `pup_ready`
...
under ordinary Workspace Manifest policy.
...
permission: SSO OIDC, portal, and AWS API effects remain under ordinary Workspace Manifest
...
`manifest show` reports `authentication.mode` as `native_workspace`. The agent
...
## Predecessor research-auth migration

The unpublished predecessor Broker state is not a Manifest desired/applied
field. `migrate apply` treats its replay-capable filesystem set as one private
authority collection: credential ciphertext plus all provider/config records,
bindings, handles, lookups, projections, and Runtime/project registries that can
make the ciphertext reachable. Exact complete enumeration, owner/type/mode,
safe-path, schema, digest, stopped-cluster, zero-attachment, and exclusive-lock
checks precede mutation; unknown, mixed, partial, corrupt, symlinked, or drifted
state fails closed.

Migration moves that complete filesystem set to an owner-only private
quarantine unknown to ordinary predecessor and final readers. Central
ciphertext/lookup state moves first. Before that move an old reader sees the
full predecessor set; at every later transaction phase it cannot resolve an old
handle, even if inert config or Workspace artifacts remain temporarily at
canonical paths. Resume completes the move and rollback restores exact bytes
only if no fresh canonical auth state exists.

On macOS the Keychain item is unchanged cryptographic recovery material, not
authorizing authentication state after filesystem ciphertext and bindings are
unreachable. Migration never asks the key provider to read, write, rotate,
rename, or delete it. On Linux, filesystem root-key material moves and restores
with the quarantined set. Public migration output contains no secret path,
provider record, handle, vault/root-key content, or Keychain-existence fact; it
may report only `research_auth_disposition: reauthentication_required`.
...
in a fresh mount-free container from the selected Workspace Manifest image. Tobari binds
...
2.1.220 at `/usr/local/bin/claude` in the selected Workspace Manifest image. A fresh
...
Terminal stdin is rejected before reading. Public Workspace Manifest/provider input,
...
Workspace Manifest/provider record and revokes all associated handles without claiming
...
| Encrypted Workspace Manifest vault | envelope 1, payload 1 | `${XDG_STATE_HOME:-$HOME/.local/state}/tobari/auth/contexts/<context-id>/vault.enc` |
...
The Workspace Manifest manifest contains no vault path, root key, primary secret, or
handle. Stable Workspace Manifest ID selects the separate encrypted vault.

Vaults use AES-256-GCM with a random 12-byte nonce and associated data binding
schema plus stable Workspace Manifest ID. Root keys stay in host storage and broker
...
1. Derive stable Workspace Manifest/project authority from the kernel-observed source
...
4. Remove one recognized handle and ask Broker for non-secret introspection of
   the full Workspace Manifest/project/provider/revision/target/header binding.
