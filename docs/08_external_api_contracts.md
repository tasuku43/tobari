...
project principal, cluster, optional caller session metadata, and optional
requested credential profile. `principal.project_id` is required to be a
...
version, cluster, scheme, fixed request port, empty-body boundary, captured body,
and credential binding pass,
...
cluster, request authority, method, path, decision, reason, credential profile
name, and upstream outcome. The project ID is metadata for host-side policy
...
configuration keys are public MVP compatibility boundaries. `principals.json`
uses schema version 1 with `bindings` containing `project_id`, `gateway_ip`,
and `network`; credential profile schema-v1 entries require a `projects` array.
Synthetic fixtures
...
unavailable-body restriction, project-bound learned rules, and
credential-profile host/project binding.
