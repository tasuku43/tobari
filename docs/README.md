...
- Provider-specific policy operations, arbitrary helper code, general TWG
  dynamic credentials, refresh, signing, companion processes, exact-client-
  version drivers, and multiple-account selection remain excluded. The sole
  reviewed built-in is static GitHub.com acquisition through fixed GitHub CLI
  argv; strict owner manifests use protected-stdin static import. See [Authentication
