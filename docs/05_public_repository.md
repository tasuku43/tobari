...
- The base recipe builds the dedicated Linux `tobari-expose` and
  `tobari-permission` helpers with a digest-pinned reviewed Go builder from the
  exact checked repository source and module closure. Both Linux architectures,
  source/snapshot equality, per-helper identity, and their licenses remain
  mechanically reviewed. The helpers are non-archive inputs to the local
  engine-native base image, not host executables, downloadable artifacts, OCI
  publications, or Formula members.
...
- [ ] The engine-native Workspace helpers' checked source/module closure,
      pinned builder, licenses, Linux amd64/arm64 construction, per-binary
      identity, and extraction evidence were reviewed; both are absent from
      host release archives and no OCI publication path exists.
